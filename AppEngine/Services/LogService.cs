...
namespace AppEngine.Services
{
    public class LogService
    {
        ...
        public static void InsertUserLogs(OperationLog type, EFContext db, int userID, int modifiedUserID)
        {
            var log = new AppLog();
        ...
        /// <param name="modifiedUser"></param>
        public static void InsertTrainingLogs(OperationLog type, EFContext db, int trainingID, int modifiedUserID)
        {
            var log = new AppLog();
            log.OperationType = type;
            var currentDate = DateTime.Now;
